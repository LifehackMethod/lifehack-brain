...
**There is a second legal answer, and it is theirs to give, not yours to take.** If they reply saying
so themselves — *"switch the project to Y"*, *"write to this other project"*, *"override the project
lock"* — the `UserPromptSubmit` hook that reads their actual prompt issues a single-use grant, and
re-running the same `arm` then succeeds once and announces it loudly. You cannot produce that grant:
saying the words yourself, or reporting that they said them, does nothing. So **ask, and let them
answer** — never hand them the sentence to recite, which turns a human gate back into your own decision.
