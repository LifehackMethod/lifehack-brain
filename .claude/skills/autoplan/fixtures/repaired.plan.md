...
> **Review:** SKIPPED 2026-09-05 · fixture regenerated by make_repaired.py; no live six-lens swarm run against this synthetic plan.
...
- [ ] **2.7 Add the retry-backoff constant to the alert emitter.** `Owner: BUILD` · gear-1
  ...
  `Repo: public · V2 · commit local, never push`
  `Verify: SHAPE` — `grep -c "RETRY_BACKOFF_SECONDS" system/tools/pulse_alert.py` → before `0`, after `1`.
...
- [ ] **2.8 Confirm the status-tile label wording.** `Owner: BUILD` · gear-1
  ...
  `Repo: public · V2 · commit local, never push`
  `Verify: SHAPE` — `grep -c "the run's suppressed count" system/tools/pulse_status.py` → before `0`, after `1`.
