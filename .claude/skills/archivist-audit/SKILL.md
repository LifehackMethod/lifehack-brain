...
## Intent (§0.5)
**User outcome:** The the notes folder drifts — dead numbered folders, orphaned files, desks missing skeleton pieces, skills out of sync, canon oversized, bare content paths breaking clone-launched sessions. archivist-audit runs the full structural sweep: 9 checks across structure, metadata, registry, drift, legacy, stale-systems, territory-map, misplaced files, and skill promise-consistency. Output: a timestamped log + an actionable proposals file. **Bar:** "I know the exact structural health of the system — nothing's silently broken or drifting."
