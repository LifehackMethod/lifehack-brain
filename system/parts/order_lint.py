...
WHY:  Law 1 -- ordering has a definite shape, so it is code's job, not a request.
      a weekly-review skill's report presented its headline pick with no year->quarter->month->week arc
...
collapsing them hides which one you have:
  ...
  BEFORE_MISSING  B appears and A never does.  The antecedent was skipped entirely --
                  this was that skill's real defect, and it is NOT a sequencing bug.
...
# The known-BAD is the REAL shape of that skill's phase 3: a ranked Win with no
# arc in front of it.  The known-GOOD is the spec-correct artifact, hand-built.
_BAD = """## Phase 3 - Prioritization

### Ranked Win (the owner-confirmed, no corrections)
1. Confirm pay split with Door 24 - before Monday Day 1.
...
This year you said: build the coaching lane into the compounding spine.
This quarter: cash in the door.
This month: executive coaching as the move-proof lane.
Which puts you HERE this week: the shoot is the container.

### Ranked Win (the owner-confirmed)
1. Confirm pay split with Door 24 - before Monday Day 1.
"""
