#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# calendar-store-sync-run.sh — CT-3 Google Calendar → durable item-store WRITE-cadence (Pulse). Off the jig.
#
# ── LLM CONTEXT ──────────────────────────────────────────────────────────────
# WHY: refreshes state/item-store/calendar/ by running calendar_store_sync.py --sync (MECHANICAL — pulls via
#      safe_calendar.py --redact, one durable record per event, delta-only, never-delete lifecycle; NO LLM,
#      no claude -p). Without a cadence the calendar store goes stale and the item-store dead-man trips.
# GUARDS: single-instance lock. gws auth is KEYCHAIN-FREE + isolated so a headless run can NEVER touch
#      the interactive login keychain. NO claude token. Writes ONLY the calendar item-store (via the
#      CT-1-guarded writer) + a machine-local proof. A write ERROR (e.g. dead auth) buzzes the phone
#      IMMEDIATELY (critical).
# REDIRECT: item store state/item-store/calendar/ (written by the python, behind the CT-1 HARD guard);
...
#      no caller. Both are
#      false — `system/pulse-config.md` carries a real `calendar-store-sync` row (86400s) invoking
#      this file. Also fixes a donor copy-paste bug: the failure-notify body named
#      "tasks_store_sync" instead of this file's own subsystem — corrected below.
# ─────────────────────────────────────────────────────────────────────────────
set -eo pipefail

SUBSYSTEM_NAME="calendar-store-sync"
STALE_AFTER_HOURS="30"                             # own absence horizon (writers run ~daily)
WATCHDOG_SECS="600"                                # a --sync pulls Google Calendar + writes records; budget generously
...
_on_exit() {
  ...
  # Drive-visible status tile (state/status/calendar-store-sync.json) — emitted on EVERY outcome so
  ...
  write_status_tile "$SUBSYSTEM_NAME" "$RC" $(( STALE_AFTER_HOURS * 3600 )) \
    "calendar store sync ok" "calendar store sync failed — store not refreshing" \
...
#    MECHANICAL (safe_calendar --redact + a deterministic writer). One-time per machine:
...
#    Probes the CALENDAR API — the one this job actually uses (calendarList list, a read verb on the
#    calendar guard's list). It used to probe gmail getProfile; on 2026-08-27 the Gmail API was
#    disabled in the creds' GCP project and that probe 403'd, standing down THIS runner (which never
#    touches Gmail) for a whole night. Probe what you use. ──
GWS_BIN="$(command -v gws 2>/dev/null || echo /opt/homebrew/bin/gws)"
PREFLIGHT_OK=0
for _attempt in 1 2 3; do
  if "$GWS_BIN" calendar calendarList list >/dev/null 2>&1; then PREFLIGHT_OK=1; break; fi
...
# ── THE WORK: refresh the calendar store (mechanical, delta-only). Watchdog kill-9s a hung pull. Output →
#    WORKLOG. Exit 0 = success; non-zero ONLY when the writer itself broke (→ trap buzzes). ──
do_work() {
  python3 "$CODE_ROOT/shared/tools/calendar_store_sync.py" --sync --verbose > "$WORKLOG" 2>&1
}
do_work & WORK_PID=$!
