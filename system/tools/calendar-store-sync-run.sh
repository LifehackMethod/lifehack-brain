...
#      the moment one exists. Also fixes a donor copy-paste bug: the failure-notify body named
...
if [ -s "$GWS_CREDS" ]; then
  ...
  echo "[$SUBSYSTEM_NAME] FATAL: no gws creds at $GWS_CREDS — export them (chmod 600) from an interactive session. See INSTALL.md."
  RC=3; exit 3
fi
