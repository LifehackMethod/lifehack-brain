...
WHY THIS FILE EXISTS AT ALL (T15.32, 2026-08-04)
------------------------------------------------
`system/hooks/guard_findings_write.sh` blocks hand-authored writes into the Hospital findings
store. Its seven patterns each end in the LITERAL store path, so they only fire when that literal
sits in the command next to the write verb. Put the path in a shell variable and every pattern
misses it:

    P="$DRIVE/<the store>/forged.jsonl"
    echo ... >> "$P"                      # <- was ALLOWED

Observed live, not theorised: a variable-path `mv "$src" "$ARCH/"` moved three shards OUT of the
store uncontested, minutes after an identical literal-path `mv` was correctly blocked.
...
this fix the inserted comments contained apostrophes, which terminated that quoting early and left
the hook syntactically invalid -- and a broken PreToolUse hook blocks EVERY Bash, Write and Edit
call, including the one that would repair it. Keeping this code in its own importable file means
the hook only ever gains an import line, never a body of new text threaded through that quoting.

Deliberately dependency-free and side-effect-free so the guard stays fast and cannot be broken by
importing it.
"""

from __future__ import annotations   # /usr/bin/python3 here is 3.9.6
