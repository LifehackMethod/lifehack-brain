#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# planning-vault-run.sh — headless runner for the Cal overnight VERBATIM raw-vault pull.
# Exports the keychain-free gws-cron env (so email_convert.py / safe_calendar.py / gws all auth
# headlessly), pre-flights gws, then runs planning-vault-pull.py. READ-ONLY against Google; the only
# writes are the local vault files. Fail-soft (a source failure → marked in _manifest, exit 0).
#
# Cron (Stage 1): 0 4 * * *  bash planning-vault-run.sh   (today's material, ready by morning)
# Test by hand:   bash planning-vault-run.sh [--date YYYY-MM-DD]
#
...
# the manifest and still exits 0), matching planning-vault-pull.py's own already-ported honest-degrade
...
. "$CODE_ROOT/system/tools/gws-auth.lib.sh" || {
  echo "[planning-vault] FATAL: cannot source $CODE_ROOT/system/tools/gws-auth.lib.sh"; exit 1; }
...
if [ -z "$DRIVE" ]; then
  echo "[planning-vault] no data root set — nothing to pull into. Set one: python3 shared/brain_root.py --set <folder>"
  exit 2
fi
...
#  vault rather than a crash — planning-vault-pull.py marks each unavailable source in its manifest.)
...
load_gws_credentials_optional planning-vault
...
if [ "$_GWS_PREFLIGHT_OK" -ne 1 ]; then
  echo "[planning-vault] WARN: gws pre-flight failed (or gws is not installed) — sources will be marked failed this run."
fi

python3 "$CODE_ROOT/system/tools/planning-vault-pull.py" "$@"; RC=$?

# ── STATUS TILE EMIT — planning-vault is a daily cron-scheduled job, not a Pulse job; stale_after_s=86400
...
_TILE="$_STATUS_DIR/planning-vault.json"
_STATUS_STR="$( [ "$RC" -eq 0 ] && echo "OK" || echo "ERROR" )"
_SUMMARY="$( [ "$RC" -eq 0 ] && echo "planning-vault pull ok" || echo "planning-vault pull failed (rc=$RC)" )"
python3 -c "
import json, os, sys
...
" 2>/dev/null || echo "[planning-vault] WARN: status tile write failed (non-fatal)"

# ── CHAIN → STAGE 2: the analysis panel runs the moment the pull lands a vault ──
# (planning-vault-pull exits 0 whenever the vault was written, even with partial sources — the
#  manifest records any failures; the panel reads the manifest and works with what's there.)
if [ "$RC" -eq 0 ]; then
  echo "[planning-vault] pull ok — chaining Stage 2 (analysis panel)…"
  bash "$CODE_ROOT/system/tools/planning-analyze-run.sh" "$@"
else
  echo "[planning-vault] pull rc=$RC — skipping Stage 2 (no/partial vault)."
fi
exit "$RC"
