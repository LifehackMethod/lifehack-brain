#!/usr/bin/env python3
"""cal-light-sweep.py — Cal's LIGHT "what's new since the 4am pull?" email check.
...
`gws gmail ... format:metadata` pass, not the heavy verbatim re-ingest (cal-vault-pull.py). Per the
cal-daily ONE-DATASET rail: the ban is on RE-PULLING EVERYTHING; a light delta sweep is fine.
...
Fail-soft: any error → honest-empty, exit 0. Usage: cal-light-sweep.py [--since YYYY/MM/DD] [--query Q] [--cap N]
"""
...
# L0 sanitize for adversarial metadata (same core email_convert/cal-vault-pull use)
...
def main():
    ...
    try:
        ...
        print(f"[cal-light-sweep] (unavailable: {e}) — nothing surfaced", flush=True)
        return 0

    msgs = listing.get("messages") or []
    if not msgs:
        print(f"[cal-light-sweep] nothing new since {cutoff} (q: {q})")
        return 0
    ...
    print(f"[cal-light-sweep] {len(rows)} new since {cutoff} (metadata only, no bodies):")
