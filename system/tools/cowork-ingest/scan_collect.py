    ...
    for fp in files:
        ...
        for r in extract_json_array(open(fp, errors="replace").read()):
            ...
            if not withheld_this:
                gist = _store_trim(gist, store_cap)   # store whole if <=cap; else cut back to a sentence boundary
...
def main():
    ...
    ap.add_argument("--store-cap", type=int, default=STORE_CAP,
                    help="max stored gist chars. Default suits SCAN's 2-3 sentence blurb; raise it for "
                         "step 2.9's EXPLORE re-read, whose whole point is a materially longer 8-12 "
                         "sentence gist that the default cap would trim away.")
    a = ap.parse_args()
