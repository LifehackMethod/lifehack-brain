...
#
# ⛔ THIS FILE MUST STAY BEHAVIOURALLY IDENTICAL TO lib/winpath_fold.sh. It is the same idea in a
# second runtime, not a variant of it. tests/test_winpath_fold_parity.sh feeds the same inputs to both
# and fails if a single output differs -- so a change here that is not mirrored there breaks the build
# rather than drifting quietly. That parity test is the reason two copies are safe.
#
...
if __name__ == "__main__":
    # `python3 winpath_fold.py <path> [force]` -- used by the parity test to drive this half exactly
    # the way the shell half is driven, so the two are compared on identical input.
