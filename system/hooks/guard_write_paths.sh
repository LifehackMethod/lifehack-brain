...
if [[ "$FILE_PATH" == "$CLAUDE_DIR/hooks/"* || "$FILE_PATH" == "$HOME/.claude/hooks/"* ]]; then
    echo '{"decision":"block","reason":"Write blocked — hook scripts are protected. WHY: Hooks are the enforcement layer; self-modification breaks the guard system. Edit manually: chmod 644 hook, edit, chmod 755. RULE: system/hook-contract.md."}'
    exit 1
fi
...
if [[ "$FILE_PATH" == "$CLAUDE_DIR/settings.json" || "$FILE_PATH" == "$HOME/.claude/settings.json" \
   || "$FILE_PATH" == "$CLAUDE_DIR/settings.local.json" || "$FILE_PATH" == "$HOME/.claude/settings.local.json" ]]; then
    echo '{"decision":"block","reason":"Write blocked — settings.json/settings.local.json is protected. WHY: Settings control permissions and hooks; agent self-modification is not permitted. Edit manually if intentional."}'
    exit 1
fi
...
if [[ "$FILE_PATH" == "$DRIVE_ROOT/system/hooks/"* ]]; then
    echo '{"decision":"block","reason":"BLOCKED: hook scripts are protected (Drive-canonical, system/hooks/). WHY: hooks are the enforcement layer; agent self-modification breaks the guard system. REDIRECT: edit via Bash only — chmod 644 the hook, edit, chmod 755. RULE: system/hook-contract.md."}'
    exit 1
fi
...
if [[ "$FILE_PATH" == "$CLAUDE_DIR/current_email_lane" || "$FILE_PATH" == "$HOME/.claude/current_email_lane" ]]; then
    echo '{"decision":"block","reason":"Write blocked — current_email_lane is set by desk launcher scripts, not agents. Launch from the correct .command file on the Desktop."}'
    exit 1
fi
...
if [[ "$FILE_PATH" == "$CLONE_ROOT/system/hooks/"* ]]; then
  printf '%s\n' '{"decision":"block","reason":"BLOCKED: clone hook scripts are protected. WHY: hooks are the enforcement layer; self-modification breaks the guard system. REDIRECT: edit via Bash only — chmod 644 the hook, edit, chmod 755. RULE: system/hook-contract.md."}'
  exit 2
fi
if [[ "$FILE_PATH" == "$CLONE_ROOT/"* ]]; then
  ...
  case "$REL" in
    ...
    |skills/*/user-*.md|skills/*/personal-*.md)
      printf '%s\n' "{\"decision\":\"block\",\"reason\":\"BLOCKED: agent-written CONTENT cannot be written into the code clone ($REL). WHY: residency P9 'who writes it' — the running agent writes content, which lives ONLY on Drive (content in git is a leak + findability risk). REDIRECT: write to \$DRIVE/$REL on the Drive _ClaudeOps tree instead.\"}"
      exit 1
      ;;
  esac
  exit 0
...
# held on the local drive OUTSIDE Google Drive (Kim, 2026-08-04: a git repo inside
...
if [[ "$FILE_PATH" == "$BRAIN_ROOT/system/hooks/"* ]]; then
  printf '%s\n' '{"decision":"block","reason":"BLOCKED: lifehack-brain hook scripts are protected. WHY: hooks are the enforcement layer; self-modification breaks the guard system. REDIRECT: edit via Bash only — chmod 644 the hook, edit, chmod 755. RULE: system/hook-contract.md."}'
  exit 2
fi
if [[ "$FILE_PATH" == "$BRAIN_ROOT/.claude/settings.json" || "$FILE_PATH" == "$BRAIN_ROOT/.claude/settings.local.json" ]]; then
  printf '%s\n' '{"decision":"block","reason":"BLOCKED: lifehack-brain settings.json/settings.local.json is protected. WHY: this file decides which hooks run at all; agent self-modification is not permitted. REDIRECT: edit via Bash if intentional, and re-read it afterwards to confirm it still parses."}'
  exit 2
fi
...
if [[ -n "$NOTES_ROOT" && "$NOTES_ROOT" == /* && "$NOTES_ROOT" != "/" && "$NOTES_ROOT" != "$HOME" && -d "$NOTES_ROOT" ]]; then
  if [[ "$FILE_PATH" == "$NOTES_ROOT/.git/"* ]]; then
    printf '%s\n' '{"decision":"block","reason":"BLOCKED: git internals under the notes root are not hand-editable. WHY: .git holds a repository own bookkeeping and a hand edit corrupts history. REDIRECT: use git commands via Bash. RULE: system/hooks/guard_write_paths.sh zone 4."}'
    exit 2
  fi
