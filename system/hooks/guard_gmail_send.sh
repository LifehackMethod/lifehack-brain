...
trap 'lhb_journal_fire "$?" "guard_gmail_send.sh" "PreToolUse" "Bash" 2>/dev/null || true' EXIT
#
# ══════════════════════════════════════════════════════════════════════════════
...
# ⭐ A SEPARATE MESSAGE FOR "I COULD NOT READ THIS". Measured 2026-08-23: this guard used to fail
# closed on an unreadable payload, a missing parser, or its own decision script crashing by
# reusing $DENY above -- so a failure that had nothing to do with sending mail came back "this
# Gmail command SENDS mail, or uses a gmail operation this guard does not recognise". Failing
# closed here is still correct; blaming a send that never happened is not.
