...
trap 'lhb_journal_fire "$?" "observability_logger.sh" "PostToolUse" "*" 2>/dev/null || true' EXIT
# ── LLM CONTEXT ──────────────────────────────────────────────────────────────
...
# repo header above says it NEVER blocks -- on a missing helper it falls back to the
