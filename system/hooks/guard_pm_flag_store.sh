...
# which is backslash-native on Windows, while the store case patterns further down are
# forward-slash literals -- so without folding a comparison copy first, neither arm would ever
# fire there and this guard would enforce nothing on Windows.
