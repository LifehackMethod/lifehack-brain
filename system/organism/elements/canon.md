...
Bash-matcher hooks (`guard_sheet_writes.sh`, `ingest_gate_enforce.sh`, etc.) do NOT fire on Write|Edit tool calls — they fire only on Bash tool calls. This means a `cp`, `tee`, or `echo >` to a canon path bypasses the entire Write|Edit hook stack, including `guard_canon_write.sh`. This is a documented, accepted gap: `guard_write_paths.sh` header explicitly states "Bash file-writes BYPASS it entirely" (KNOWN-GAP accepted 2026-07-14). Mitigation is discipline + review.
