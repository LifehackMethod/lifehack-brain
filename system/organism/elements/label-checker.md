...
>
> ⛔ `system/hooks/guard_organism_map.sh` — never ships, DROPPED by ruling. `system/tools/organism/label_manifest.yaml` lines 31-38 records the decision verbatim: three donor guards *"DO NOT EXIST HERE AND STRUCTURALLY CANNOT — their rows are DROPPED, not carried over as a permanently-red TARGET."* Its fire-test row is gone from the manifest (line 173-175), so nothing here claims it is enforced.
>
