...
gap_disposition_note: "authored 2026-08-04, AFTER the 2026-07-28 class-level ruling batch — Hospital did not exist then, so this is NOT one of that day's rulings. ★ RAISED PARTIAL→LIVE 2026-08-04 (T15.30), EARNED FROM CODE not hand-typed: the two gaps this note previously cited are both closed — the deny-hook exists and is registered (system/hooks/guard_findings_write.sh, settings.json:293), and detector coverage is complete. Machine check: every system/tools/*-health.py plus organism/organism-health.py now calls emit_finding; the ONLY file that does not is backlog-health.py, a CUT approved by the operator because backlog_groom.py:290 already emits findings off the identical build_report() and the health script is merely its tile renderer. All 13 producers resolve a cadence in findings_deadman.load_producer_roster(). ⚠ PORT-DELTA CORRECTION (2026-08-16, THIS REPO ONLY — READ BEFORE THE 'CORRECTED 2026-08-05' TEXT BELOW): everything from 'CORRECTED 2026-08-05' through '...T15.31.' further down is verbatim DONOR history (claudeops-config) that rode along when this element was ported — true THERE, but FALSE HERE from the moment of porting until today: the fix it describes, system/tools/hook_path_resolve.py, never migrated (confirmed absent by a direct find), so guard_findings_write.sh had been silently degrading to literal-path-only matching on THIS repo the entire time — exactly the ·gap the donor paragraph below says was closed. ⛔ that donor claim, 'THE VARIABLE-PATH BYPASS IS CLOSED', was NOT true of this repo. THIS IS THE NOTE'S OWN NAMED LESSON, INVERTED, ON ITSELF: the paragraph below is a defect-fix note left standing after the fix it announces failed to migrate — the exact harm its own '★ THE LESSON' sentence warns about ('sends the next session to re-fix working code'), aimed the other way: a session reading this reasonably believed the gap was closed and moved on, instead of closing it. CLOSED HERE 2026-08-16: hook_path_resolve.py ported unchanged (dependency-free stdlib re, no repo-specific paths — only its own docstring gained a provenance line); guard_findings_write.sh needed NO edit, because its own 2026-08-14 port note already says the sys.path entry resolves relative to this repo and will start working automatically the day someone ports hook_path_resolve.py, no second edit needed — and that held exactly as designed. VERIFIED LIVE HERE 2026-08-16, before/after, by firing the REGISTERED guard: BEFORE, a variable-path payload (F=\"state/findings/x.jsonl\"; echo bad >> \"$F\") got rc=0 (allowed — the live bug) while an identical literal-path payload correctly got rc=2; AFTER, the same variable-path payload gets rc=2 (blocked), a two-hop variable chain across newlines also blocks, a literal-path write is unchanged at rc=2, and a legitimate emit_finding.py call still passes at rc=0. The donor paragraph immediately below is left verbatim as history, not current-state-at-this-destination — begins at 'CORRECTED 2026-08-05', ends at 'T15.31.': CORRECTED 2026-08-05 — THE VARIABLE-PATH BYPASS IS CLOSED AND THIS NOTE WAS STILL CLAIMING IT OPEN. This note used to read: 'THE ·gap IS NOT COSMETIC — guard_findings_write.sh matches the LITERAL string state/findings/, so a path held in a shell VARIABLE evades every one of its seven patterns... still open behind one line of indirection. Unfixed deliberately: a hook edit needs the SOP receipt and the operator's sign-off.' That was true when written (T15.32, found at the T15.30 review) and was FIXED later the same day: guard_findings_write.sh:88-96 now imports system/tools/hook_path_resolve.py and matches the RESOLVED command, exactly as the ruling demanded ('a path-matching guard cannot be completed by adding an eighth regex'). VERIFIED LIVE 2026-08-05 by firing the REGISTERED guard with a variable-path payload: rc=2, blocked. The resolver degrades loudly rather than denying if it is ever missing, so a resolver outage cannot black out every tool call. ★ THE LESSON IS THIS PROJECT'S OWN SIGNATURE ONE, AGAIN: a defect note left standing after the defect was fixed sends the next session to re-fix working code — the exact inverse of T15.26/T15.27, and caught here only because T18.6b re-read the guard before extending it. STILL GENUINELY OPEN (donor repo, moot at this destination — this repo is one-machine by design, see the PORT BANNER's machine_token.py line at the top of this element): the guard is watched firing on the primary machine ONLY (the second machine dark since 2026-07-04) — T15.31."
generated_from:
