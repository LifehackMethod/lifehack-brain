...
| 12 | **Git hooks (pre-commit)** | `core.hooksPath` → `system/githooks/pre-commit` | Git-versioned (hook lives in the clone; activation is a local `git config`) | `git config --get core.hooksPath` returns an ABSOLUTE path ending `/system/githooks` (a relative value fails OPEN from any worktree checked out before a hook existed — measured 2026-08-30); if not: `git config core.hooksPath "$(pwd)/system/githooks"` (also run by `bootstrap-machine.sh` Step 8) | VERIFY |
