...
- **Deploying it:** both halves are files in this repo — the script at `system/hooks/`, the registration in `.claude/settings.json` using `${CLAUDE_PROJECT_DIR}` — so both travel by `git pull` and neither is re-done by hand. ⛔ **Then RESTART.** The harness reads `settings.json` at session start, so a newly registered hook does nothing at all in the window that registered it, and the only symptom is silence. That is the single commonest reason a correct hook appears not to work. (→ `system/hook-contract.md`, Deploy & Verify.)
  > ⚠ **CORRECTION — MEASURED 2026-08-27. "Does nothing at all in the window that registered it" is NOT universally true on the current harness.** `guard_gh_pr_merge.sh` was written, registered into `~/.claude/settings.json`, and **fired roughly a minute later, in the same window, with no restart** — twice, on real commands, denying by its own name and path. The registration was USER-scope (`~/.claude/settings.json`), not the project-scope `.claude/settings.json` this line is about, which may be the whole difference.
  > ⛔ **DO NOT read this as "restart is unnecessary."** The corrected rule is narrower and cuts the other way: **a restart is still the only way to be SURE, and a hook that has not fired is not evidence either way** — you cannot conclude "it needs a restart" from silence, because silence is also what a broken hook looks like. **Watch it fire; never infer its state from a rule about when settings are read.**
  > ⚠ Kept beside the original rather than replacing it — the original is right about project-scope registration and about restart being the reliable path, and this project's own doctrine is that a wrong fact gets a visible correction, never a silent overwrite.
